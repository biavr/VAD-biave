...
class VAD(MVXTwoStageDetector):
    ...
    def forward(self, inputs, data_samples=None, mode='loss', **kwargs):
        """Standard MMEngine forward interface natively mapped to VAD requirements.
        
        Args:
            inputs (dict | torch.Tensor): If dict, contains 'img'.
            data_samples (list[:obj:`Det3DDataSample`]): Annotations and metas.
            mode (str): 'loss' for training, 'predict' for testing.
        """
        # 1. Safely extract the image tensor at the root level
        ...
        if mode == 'loss':
            if img is None:
                raise RuntimeError("The 'img' tensor is None. Check if the key in nuscenes_vad_dataset.py matches 'img'.")
            return self.forward_train(img=img, data_samples=data_samples, **kwargs)
            
        # 3. Evaluation/Testing Workflow
        elif mode == 'predict':
            img_metas = None
            gt_bboxes_3d = None
            gt_labels_3d = None
            ...
            if data_samples is not None and isinstance(data_samples, list) and len(data_samples) > 0:
                ...
                # Retrieve the real img_metas generated by the dataset pipeline
                    ...
                    for target_meta in meta_list:
                        if isinstance(target_meta, dict):
                            ...
                            if 'pad_shape' in target_meta:
                                ...
                                if isinstance(s, (tuple, list)) and not isinstance(s[0], (tuple, list)):
                                    target_meta['pad_shape'] = [[s[0], s[1], 3] for _ in range(6)]
